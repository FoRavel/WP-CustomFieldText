...
class RWMB_Fieldset_Text_Field extends RWMB_Field {
	...
	public static function html( $meta, $field ) {
		...
		foreach ( $field['options'] as $key => $label ) {
			...
			if($key === 'date'){
				$html[] = sprintf( $tpl, $label, "<input type='date'  class='rwmb-fieldset_text' name='prefix-fieldset_text_1[$i][date]'>");
			}elseif($key === 'id'){
				$html[] = sprintf( $tpl, $label, "<input type='number' class='rwmb-fieldset_text'  name='prefix-fieldset_text_1[$i][id]'>");
			}
			$i++;
		}
